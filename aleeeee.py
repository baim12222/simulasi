...
def power():
    current = entry.get()
    try:
        base, exponent = map(float, current.split('^'))  # expects input like "2^3"
...
button_1 = tk.Button(master=frame, text='1', padx=15,
            pady=5, width=3, bg='grey', fg='white', command=lambda:
myclick(1))
...
button_power = tk.Button(master=frame, text="^", padx=15,
                        pady=5, width=3, bg='#004d4d', fg='white', command=power)
button_power.grid(row=5, column=3, pady=2, sticky="nsew")
